...
def createfile(name,lst):
    with open(f'{path}/{name}','a',newline='')as f:
        script= csv.writer(f)
        script.writerow(lst)
        print(f"{name} File has been sucessfully updated!")
...
def grade(num):
    if num>=90:
        return("Outstanding Results- Grade A.")
    elif num<90 and num>=80:
        return("Excellent Results- Grade B.")
    elif num<80 and num>=70:
        return("Good Results- Grade C.")
    elif num<70 and num>=60:
        return("Average Results- Grade D.")
    elif num<60 and num>=50:
        return("Below average  Results- Grade E.")
    else:
        return("Extremely poor Result- Grade F.")
...
print("Please write the stream names in short form and in capital letters as mentioned above!")
print()
...
for i in range(student_no):
    ...
    batch=input("Which batch the student is in (e.g. 2022-26) : ")
    ...
    if duplicate('Student.csv',student_id,0):
        ...
        print()
        print("The subjects are [Python,Math,Physics,Chemistry,Biology,ESP & SDP]")
        ...
        with open(f"{path}/ReportCards/{student_id}_{''.join(name.split())}.txt",'w') as f:
            ...
            f.writelines([f'Name of the student : {name} \n',
                          ...
                          f'Marks obtained in ESP & SDP is : {marks_lst[5]} \n'])
        ...
        ask=input("Do you want to remove this name from database (Y/N) : ")
